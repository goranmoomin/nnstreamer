...
ncnn_subplugin::configure_instance (const GstTensorFilterProperties *prop)
{
  ...
  if(prop->num_models == 1) {
    if (net.load_param_bin (prop->model_files[0]))
      throw std::invalid_argument ("Failed to open the file " + std::string (prop->model_files[0]));
  } else if (prop->num_models == 2) {
    if (net.load_param (prop->model_files[0]))
      throw std::invalid_argument ("Failed to open the file " + std::string (prop->model_files[0]));
    if (net.load_model (prop->model_files[1]))
      throw std::invalid_argument ("Failed to open the file " + std::string (prop->model_files[1]));
  } else {
...
void extract_thread (ncnn::Extractor &ex, const int idx, ncnn::Mat &out) {
  ex.extract (idx, out);
}

void input_thread (ncnn::Extractor &ex, const int idx, const ncnn::Mat &in,
