...
class ncnn_subplugin final : public tensor_filter_subplugin
{
  ...
  int getModelInfo (model_info_ops ops,
...
ncnn_subplugin::configure_instance (const GstTensorFilterProperties *prop)
{
  ...
  gst_tensors_info_copy (
    std::addressof (inputInfo), std::addressof (prop->input_meta));
  gst_tensors_info_copy (
    std::addressof (outputInfo), std::addressof (prop->output_meta));

  // check number of model files
  if (prop->num_models > 2 || prop->num_models <= 0) {
    throw std::invalid_argument (
      std::string ("Number of model files must be 1 or 2;") +
  ...
  for (guint i = 0; i < inputInfo.num_tensors; i++) {
    ...
    while (*dim) shape.push_back (*dim++);

    // make ncnn matrix object
    ncnn::Mat in;
    switch (shape.size()) {
      case 1: in = ncnn::Mat (shape[0]); break;
      case 2: in = ncnn::Mat (shape[0], shape[1]); break;
      case 3: in = ncnn::Mat (shape[0], shape[1], shape[2]); break;
      case 4: in = ncnn::Mat (shape[0], shape[1], shape[2], shape[3]); break;
      default: throw std::invalid_argument ("Wrong input dimension");
    }
...
ncnn_subplugin::invoke (const GstTensorMemory *input, GstTensorMemory *output)
{
  ...
  for (guint i = 0; i < inputInfo.num_tensors; i++) {
    ...
    input_thrs.emplace_back (input_thread,
      std::ref (ex), input_indexes.at (i), std::ref (in),
      input_data, num_bytes);
    input_data += num_bytes;
  }

  // join threads
  for (std::thread &thr : input_thrs) thr.join();
  ...
  if (use_yolo_decoder) {
    ...
    for (guint i = 0; i < outputInfo.num_tensors; i++) {
      ...
      output_thrs.emplace_back(extract_thread,
        std::ref (ex), output_indexes.at (i), std::ref (out),
        nullptr, 0);
    }
    ...
    // join threads
    for (std::thread &thr : output_thrs) thr.join ();
 ...
 */
int
ncnn_subplugin::getModelInfo (model_info_ops ops,
    GstTensorsInfo &in_info, GstTensorsInfo &out_info)
{
  switch (ops) {
    case GET_IN_OUT_INFO:
      gst_tensors_info_copy (
        std::addressof (in_info), std::addressof (inputInfo));
      gst_tensors_info_copy (
        std::addressof (out_info), std::addressof (outputInfo));
      break;
    case SET_INPUT_INFO:
    default:
      return -ENOENT;
 ...
 */
int
ncnn_subplugin::eventHandler (event_ops ops,
...
ncnn_subplugin::parseCustomProperties (const GstTensorFilterProperties *prop)
{
      ...
      if (g_strv_length (option.get ()) == 2) {
        ...
        if (g_ascii_strcasecmp (option.get ()[0], "use_yolo_decoder") == 0) {
          ...
          if (g_ascii_strcasecmp (option.get ()[1], "true") == 0) {
            ...
            throw std::invalid_argument (
              "Invalid option for use_yolo_decoder: "
          ...
          if (g_ascii_strcasecmp (option.get ()[1], "true") == 0) {
            ...
            throw std::invalid_argument (
              "Invalid option for use_vulkan: "
              + std::string (option.get ()[1]) + ".");
          }
        } else {
          throw std::invalid_argument (
              "Unsupported custom property: "
              + std::string (option.get ()[0]) + ".");
        }
      } else {
        throw std::invalid_argument (
          "Unsupported custom property: "
          + std::string (options.get ()[i]) + ".");
      }
    }
  }
  ...
  if (vulkan_option == "auto") {
    if (ncnn::get_gpu_count () != 0) vulkan_option = "true";
    else vulkan_option = "false";
  }

  // set member variable
  if (vulkan_option == "true") use_vulkan = true;
  else use_vulkan = false;
}
